            ...
            if (command < Commands.SECOND_IF_BOUNDARY) {
                ...
                if (command < Commands.FIRST_IF_BOUNDARY) {
                    ...
                    if (command == Commands.V2_SWAP_EXACT_IN) {
                        ...
                        assembly {
                            recipient := calldataload(inputs.offset)
                            tokenIn := calldataload(add(inputs.offset, 0x20))
                            amountIn := calldataload(add(inputs.offset, 0x40))
                            amountOutMin := calldataload(add(inputs.offset, 0x60))
                            // 0x80 and 0x100 offset is the path, decoded below
                            payerIsUser := calldataload(add(inputs.offset, 0xC0))
                        }
                        address[] calldata path = inputs.toAddressArray(4);
                        uint24[] calldata fee = inputs.toUint24Array(5);
                        address payer = payerIsUser ? lockedBy : address(this);
                        v2SwapExactInput(map(recipient), tokenIn,amountIn, amountOutMin, path, fee, payer);
                        ...
                        assembly {
                            ...
                            amountInMax := calldataload(add(inputs.offset, 0x60))
                            // 0x60 offset is the path, decoded below
                            payerIsUser := calldataload(add(inputs.offset, 0xC0))
                        }
                        address[] calldata path = inputs.toAddressArray(4);
                        uint24[] calldata fee = inputs.toUint24Array(5);
                        address payer = payerIsUser ? lockedBy : address(this);
                        v2SwapExactOutput(map(recipient),tokenOut, amountOut, amountInMax, path,fee, payer);
